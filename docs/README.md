...
- mouse wheel scroll moves the focused pane history viewport; pressing any key returns the focused pane to follow mode at the live cursor. ANSI foreground/background styles are preserved while viewing scrolled history, and cursor mode snapshots keep those styles too. cursor mode is available via `prefix+[`/`enter-cursor-mode` (`copy-mode` remains an alias), uses `v` to toggle anchor, `x` for linewise select/extend, `w/b/e` to set anchor while word-moving, plain cursor motions clear anchor, `w/b/e` use gargo-like `word/other/whitespace` classes across line boundaries, and supports gargo-like `g` chords (`gg` file start, `ge`/`G` file end, `gh` line start, `gl` line end). `/` opens a gargo-style scrollback search bar (also `prefix+/`/`search-scrollback` straight from normal mode): typing searches incrementally forward from the cursor with wrap-around, case-insensitively; Enter confirms and keeps viewport matches highlighted (current match on the cursor is distinct) for `n`/`N` next/prev stepping; Esc cancels and restores the pre-search cursor/viewport; Up/Down or Ctrl+p/n browse per-client search history
...
- releasing a drag keeps the selection highlighted instead of copying it; `prefix+y`/`copy-selection` copies the highlighted text (also exposed in the command palette). The selection drops on the next click, on a key press destined for the pane, or when a mouse-reporting guest consumes a fresh press. A plain click that never grew a range leaves no selection
- `cmd+c` (super+c, canonical binding key `D-c`) is a default global `copy-selection` binding: with a highlighted mouse selection it copies (per-client OSC 52 for remote/SSH clients, native clipboard for the local client); without one the key falls through to the pane. Requires a host terminal that reports super-modified keys via the kitty keyboard protocol and forwards cmd+c when it has no selection of its own (e.g. ghostty). Binding specs accept `cmd`/`command`/`super`/`win`/`d` modifier tokens; the last `-`/`+`-separated token is always the key, so `C-c`/`cmd+c` parse correctly. Super-modified chars with no binding are dropped instead of typing the bare letter (no legacy encoding exists; kitty-protocol panes still receive CSI u)
- rapid multi-clicks expand the selection gargo-style from a fixed origin cell: the second click selects the word under the cursor (`word/other/whitespace` classes, wide-char continuation cells inherit the owner cell's class), further clicks grow to the smallest strictly-larger unit (non-whitespace run, then the trailing-blank-trimmed line). The chain window is 400ms with a 1-cell radius; drag jitter inside the expanded range keeps the selection, dragging beyond it extends from the expanded start
